...
- **VIP 解析（可选）**：播放器工具栏注入皇冠按钮，点击后用第三方公开解析站（jx.xmflv.com）原地替换官方播放器；登录浮层、观看历史等官方功能不受影响
...
> **VIP 解析说明**：内置于 `src/preload.js` 的可选功能（开关 `OPT.vipParser`，默认开启），移植自 Tampermonkey 社区脚本「Tencent VIP」v0.3。点击播放器工具栏的皇冠按钮后，当前播放器会被替换为第三方公开解析站 `jx.xmflv.com` 的 iframe；切集/换页自动恢复官方播放器。解析站为外部第三方服务，客户端本身不做任何解密或流抓取；其可用性、安全性与内容合规性由该第三方决定，请自行评估风险。不需要时把 `OPT.vipParser` 改为 `false` 即可完全关闭。
...
- 优化项开关：`src/preload.js` 顶部的 `OPT`（弹幕隐藏、水印移除、片头广告跳过、VIP 解析等）
